...
- string (a sequence of Unicode [code points](https://unicode.org/glossary/#code_point). Note that a code point is distinct from a [code unit](https://unicode.org/glossary/#code_unit))
...
`unknown-function` error has occurred. How and when this error is raised is
...
is not provided, then implementations must indicate to the caller that an
...
Function expressions are allowed within sub expressions.
...
to enforce homogeneous types. The subtype is surrounded by
...
there exists within the `$subject` string at least one occurrence of the `$search` string.
If the `$search` value is not a string, the function MUST return `false`.
...
- number/array/object/boolean - The JSON encoded value. The
...
depend on implementation details. For an implementation to be compliant it
needs to indicate that an error occurred while attempting to evaluate the
JMESPath expression.
